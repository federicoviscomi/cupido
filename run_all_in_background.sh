...
rmiregistry -J-classpath -J"$CLASSPATH" &>rmiregistry.log &
...
java -classpath "$CLASSPATH" unibo.as.cupido.backend.gtm.GlobalTableManager &>gtm.log &
java -classpath "$CLASSPATH" unibo.as.cupido.backend.gtm.GlobalChatImpl &>chat.log &

PIDS="$PIDS $!"

sleep 2

java -classpath "$CLASSPATH" unibo.as.cupido.backend.ltm.LocalTableManager &>ltm.log &
