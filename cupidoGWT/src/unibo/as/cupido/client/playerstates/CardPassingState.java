	...
	public CardPassingState(final CardsGameWidget cardsGameWidget, final PlayerStateManager stateManager, final List<Card> hand) {
		...
		okButton.addClickHandler(new ClickHandler() {
			...
			public void onClick(ClickEvent event) {
				...
				cardsGameWidget.runPendingAnimations(1500, new GWTAnimation.AnimationCompletedListener() {
					@Override
					public void onComplete() {
						...
						final Comparator<Card> cardComparator = CardsGameWidget.getCardComparator();
						Collections.sort(sortedList, cardComparator);
						
						// Uncover the cards in reverse order, to respect the precondition
						// for CardsGameWidget.coverCard().
						for (int i = 0; i < 3; i++)
							cardsGameWidget.coverCard(0, sortedList.get(3 - i - 1));
		...
		cardsGameWidget.setListener(new GameEventListener() {
			...
			public void onCardClicked(int player, Card card, State state,
				...
				cardsGameWidget.runPendingAnimations(200, new GWTAnimation.AnimationCompletedListener() {
					@Override
					public void onComplete() {
					}
				});
