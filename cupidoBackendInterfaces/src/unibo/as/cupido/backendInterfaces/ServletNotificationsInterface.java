 ...
 * This interface contains method to notify the servlet of some events
 * 
 * Note: servlet in not notified when a player enter the table to observe the match
 */
public interface ServletNotificationsInterface extends Remote {

	/**
	 * End of the game is notified to the servlet.
	 * @param matchPoints points the player has taken during this hand
	 * @param playersTotalPoint points of the player updated after this match
	 */
	 ...
	 * @param cards the starting hand of the player
	 */
	 ...
	 * @param message
	 * @throws RemoteException is message can't be send
	 */
	public void notifyLocalChatMessage(ChatMessage message) throws RemoteException;

	/**
	 * Notify that a player has passed 3 card to the player connected with the current servlet
	 * @param cards card passed. Card.lenght must be 3
	 ...
	 * @param playerPosition table position of the player which has played the card
	 * @throws RemoteException
	 */
	public void notifyPlayedCard(Card card, int playerPosition) throws RemoteException;

	/**
	 * Servlet if notified when a player enter the table
	 * @param name
	 * @param isBot false if the player is a human player, true otherwise
	 * @param point points of the player, if isBot==true, point is meaningless
	 * @param position table position where the player has entered
	 */
